    ...
    setInterval(async () => {
      ...
      if (alluser !== null) {
        alluser.forEach(async v => {
          const waktumasuk = DateTime.fromFormat(v.jammasukwa, 'HH:mm:ss').plus(
            {minute: 30},
          );
          ...
          if (
            reminder.toFormat('HH:mm') === waktusekarang &&
            v.reminder === 'true'
          ) {
            await sock.sendMessage(`${v.hp}@s.whatsapp.net`, {
              text: 'apakah kamu masih terhubung ?',
            });
      ...
      if (events['messages.upsert']) {
        ...
        if (messages.type === 'notify') {
          (await Badword.all()).forEach(async v => {
            const kj = message[0].message?.conversation
              ?.toLowerCase()
              .includes(v.word);
            if (kj)
              return await sock.sendMessage(message[0].key.remoteJid!, {
                text: 'badword detected',
              });
          });
          ...
          if (ava === null) {
            ...
            await Usersession.create({
              ...
              await sock.sendMessage(message[0].key.remoteJid!, {
                text: 'Terima kasih telah menghubungi Perguruan Panca Budi. Silakan ketik "Menu" untuk Informasi Administrasi Keuangan Sekolah',
              });
              return;
            });
          } else {
            ...
            if (
              ava.tanggalmasukwa === null ||
              tgl.toISODate() !== ava.tanggalmasukwa?.toISODate()
            ) {
              await Usersession.query()
                .where('hp', hp!)
                .update({
          ...
          const ceksesi = await Usersession.query()
            .where('hp', hp!)
            .where('tanggalmasukwa', tgl.toISODate())
            .where('active', 'true')
            .first();
          ...
          if (message[0].message?.conversation === '7') {
            await Usersession.query()
              ...
              .finally(async () => {
                ...
                await sock.sendMessage(message[0].key.remoteJid!, {
                  text: 'terima kasih sudah menggunakan layanan ini ...',
                });
                return;
              });
          }
          ...
          if (message[0].message?.conversation?.match(/menu/i)) {
            ...
            await sock.sendMessage(message[0].key.remoteJid!, {
              text:
                'Informasi pembayaran administrasi sekolah perguruan panca budi \n\n' +
                arr.join('\r\n'),
            });
          }
          ...
          if (message[0].message?.conversation?.match(/^[1-6]{1}$/i)) {
            ...
            if (ceksesi === null) {
              await sock.sendMessage(message[0].key.remoteJid!, {
                text: 'masukkan nama nis dulu ya ...',
              });
              return;
            }
            ...
            if (cek?.isaskdb === 'true') {
              if (cek.command === 'infotagihan') {
                const tagihan = await Tagihansiswa.query()
                  .select(
                    Database.rawQuery('*, SUM(tagihanjumlah) AS totaltagihan'),
                  )
                ...
                tagihan.forEach(v => {
                  tm.push(
                    `${v.tagihantermin} ${v.$extras.totaltagihan} ${
                      v.tagihanaktif === '1' ? 'belum' : 'lunas'
                    }`,
                  );
                });
                await sock.sendMessage(message[0].key.remoteJid!, {
                  text: `Noreg ${ceksesi.noreg}\nNama ${
                    ceksesi.nama
                  }\n${tm.join('\r\n')}`,
                });
              }
              ...
              if (cek.command === 'inforincian') {
                const tagihand = await Tagihansiswa.query()
                  .select(
                    Database.rawQuery(
                      'tagihantermin, tagihanperiode, SUM(tagihanjumlah) as total, GROUP_CONCAT((SELECT nmbiaya FROM tagihan_jenis_biaya WHERE kdbiaya = tagihanKodeBiaya), " ", tagihanjumlah SEPARATOR "\n") as biaya',
                    ),
                  )
                  .where('tagihansiswaid', ceksesi.noreg)
                  .where('tagihanperiode', DateTime.now().year)
                ...
                if (tagihand === null) {
                  await sock.sendMessage(message[0].key.remoteJid!, {
                    text: 'Tidak ada tagihan periode ' + DateTime.now().year,
                  });
                  return;
                }
            ...
            if (cek?.isimage === 'false' && cek.isaskdb === 'false') {
              if (cek.command === 'infogabung') {
                ...
                let arrt: string[] = [];
                cpkepsek.forEach((v: {nama: string; nohp: string}) =>
                  arrt.push(`${v.nama} ${v.nohp}`),
                );
          ...
          if (
            message[0].message?.conversation?.match(
              /^\w+( \w+)*\s([0-9]{2}).([\.0-9]{2,8})$/i,
            ) &&
            ceksesi === null
          ) {
            ...
            const siswa = await Siswa.query().where('nis', ns).first();
